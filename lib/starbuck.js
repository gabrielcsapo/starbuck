 ...
 * @param {Object} [opts.npm] npm configuration options
 */
async function getVersionInfo (name, opts) {
  const data = await pacote.packument(name, opts)

  const { versions: versionsInfo } = data
  const versions = Object.keys(versionsInfo)

  return { current: data['dist-tags']['latest'], versions: versions }
}
